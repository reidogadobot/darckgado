...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:nabuto\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=559384051080:+55 93 8405-1080\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD NABUTO BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo do meu dono. Registre-se para fazer amizade com o bot Nabuto por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar nabuto|14\n\n──「 Nabuto BOT 」──`,
			...
			const ownerNumber = ["559384051080@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"559384051080@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["559384051080@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["559384051080@s.whatsapp.net"]//ubah nomor lo
			const premium = ["559384051080@s.whatsapp.net","553188115277@s.whatsapp.net","5511964352259@s.whatsapp.net","556581415666@s.whatsapp.net","559887583208@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('pow cara, não fale essas coisas só pq vc é adm, isso pode ofender alguém e fazer a pessoa ficar depremida😣')
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isAntiRacismo) return reply('função antiracismo on')
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'owner':
			    case 'dono':
                    client.sendMessage(from, {displayname: "Zoro", vcard: vcard}, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "Zoro"🙄, chama ele aí ele diz que é "Shitpost"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem?', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Cade o texto seu burro?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto ï¿½ tio')
